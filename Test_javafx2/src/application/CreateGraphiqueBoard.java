...
public class CreateGraphiqueBoard extends Application {
	   ...
	   public CreateGraphiqueBoard() {
	      // Initialise les membres
	       ...
	       title.setPrefWidth(959.04); // largeur de la fen�tre
		   ...
		   choiceBox.setValue("5D"); // d�finit la valeur par d�faut
	      ...
	      // Cr�er les lignes horizontal et vertical
	      ...
	      // Ajoutez les �l�ments � anchor ici
	      ...
	      // Cr�ez la sc�ne et affichez la fen�tre ici
	   ...
	   // M�thode pour cr�er les lignes horizontal
	   ...
	   // M�thode pour cr�er les lignes vertical
	   ...
	   private void setMouseEvent_cicle() {
		    for (Circle circle : intersectionPoints) {
		        ...
		        if(!containsPoint2D(LineIntersectionDrawer.Point_depart(), point)) {
		            // Cr�e un �v�nement qui se d�clenche lorsque l'utilisateur clique sur le cercle
		            circle.setOnMouseClicked(event -> {
		                ...
		                if (GameMechanics.playMove((int)point.getX(), (int)point.getY(), board)) {
		                    // Modifie l'opacit� du cercle
		                	System.out.println("vrai");
		                    circle.setOpacity(circle.getOpacity() == 0 ? 1 : 0);
		                    // Cr�e un canvas de la m�me taille et position que le cercle
	   ...
	   public static void linkBoard(Board board1) { // ajoutez cette fonction � votre classe
		    board = board1;
		}
