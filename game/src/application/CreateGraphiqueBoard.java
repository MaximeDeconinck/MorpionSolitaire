...
/**
 * This class represents the graphical version of the Morpion Solitaire game. It
 * displays a grid of circles, representing the intersections of lines, where
 * the player can place their game pieces. It also includes a label for the
 * current state of the game and buttons for making moves and getting hints.
 */
			...
			if (!containsPoint2D(LineIntersectionDrawer.Point_depart(), point)) {
				circle.setOnMouseClicked(event -> {
					...
					if (GameMechanics.playMove((int) point.getX(), (int) point.getY(), board)) {
						...
						canvas.getGraphicsContext2D().fillText(Integer.toString(counter), circle.getRadius(),
								circle.getRadius());
	...
	*/
	...
	private void searchSolution() {
		...
		while (!GameMechanics.isGameOver(board)) {
			...
			targetCircle.fireEvent(new MouseEvent(MouseEvent.MOUSE_CLICKED, 0, 0, 0, 0, MouseButton.PRIMARY, 1, true,
